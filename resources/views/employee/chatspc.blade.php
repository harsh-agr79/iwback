...
<div class="main-chat-ui">
        ...
        <div class="cd-list-section">
            ...
            {{-- {{$item->id}} --}}
                ...
                if ($item->rid == $user[0]->id) {
                    ...
                    $cmpy = DB::table('companies')->where('id', $chaterid)->first();
                    $sid = $item->sid;
                    $rid = $item->rid;
                ...
                @endif
                <a href="{{url('candidate/messages/'.$chaterid)}}" class="black-text">
                <div class="cd-list-item {{$act}}">
                    @if ($cmpy->cmpydp == NULL)
                    <img src="{{asset('assets/pngs/company.png')}}" class="cd-dp">
                    @else
                    <img src="{{asset('company/dp/'.$cmpy->cmpydp)}}" class="cd-dp">
                    @endif
                    <div class="cd-info">
                        <h1 class="cd-name">{{$cmpy->cmpyname}}</h1>
    ...
    <div class="chat-msg-section">
        ...
        @else
        @php
        $cmpy2 = DB::table('companies')->where('id',$active)->first();   
       @endphp
       <div class="msg-header">
           <div class="msg-info">
               @if ($cmpy2->cmpydp == NULL)
                   <img src="{{asset('assets/pngs/company.png')}}" class="cd-dp">
                   @else
                   <img src="{{asset('company/dp/'.$cmpy2->cmpydp)}}" class="cd-dp">
                   @endif
    ...
    <div class="chat-settings-section">
        ...
        @else
        @if ($cmpy2->cmpydp == NULL)
        <img src="{{asset('assets/pngs/company.png')}}" class="cd-dp-set">
        @else
    ...
    <script>
        ...
        $(function(){
            ...
            socket.on('sendChatToClient', (message) => {
                ...
                else if(message.messages[0].rid == $('#sid').val() && message.messages[0].sty == 'cmpy')
                {
                    // $(`#${message.messages[0].rid}`).text(`${message.messages[0].msg}`)
                    $(`#${message.messages[0].sid}`).text(`${message.messages[0].msg}`)
                }
